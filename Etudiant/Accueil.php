...
$id=$_SESSION['login'];
 ...
 $req=$con->prepare("SELECT * FROM `etudiant` INNER JOIN `classe` ON `etudiant`.`FK_ID_CLASSE`=`classe`.`ID_CLASSE` WHERE
  `Matricule`='$id' ");
  $req->execute();
  $data = $req->fetchAll();


 $req2=$con->prepare("SELECT COUNT(*) FROM `etudiant` INNER JOIN `ec` ON `etudiant`.`FK_ID_CLASSE` = `ec`.`FK_ID_CLASSE` WHERE
  `Matricule`='$id' ");
  $req2->execute();
  $data2 = $req2->fetchAll();
                    ...
                    <div class="col-lg-6">
                        <div class="text-container">
                            <h1><span class="turquoise"><?php
                                                foreach ($data as $key ){ echo 'Bienvenue '.$key['Nom_Etud'].' '.$key['Prenom_Etud'];} ?></span></h1>
            ...
            <div class="row">
              <div class="col-lg-1"></div>
                <div class="col-lg-6">
                    <div class="text-container">
                        <h2>Vous êtes en <?php
                                                foreach ($data as $key ){ echo $key['FK_ID_CLASSE'];} ?> </h2>
                        <p><?php
            ...
            <div class="row">
              <div class="col-lg-1"></div>
                ...
                <div class="col-lg-5">
                    <div class="text-container">
                        <h2> Vous avez <?php 
                                              foreach ($data2 as $key ){ echo $key['COUNT(*)'];} ?> matières à valider cette année</h2>
