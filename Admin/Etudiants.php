...
$id=$_SESSION['login'];
 ...
 $req2=$con->prepare("SELECT `ID_CLASSE`,`NIVEAU` FROM `classe` ORDER BY `NIVEAU` ASC ");
  $req2->execute();
  $data2 = $req2->fetchAll();
...
<body class="animsition">
  ...
  foreach ($data2 as $key ) {if ($id) {
   ...
   <form method="POST" action="interetud.php">
                                                ...
                                                <tr>
                                                <td><input type="text" name="" value="'.$key['ID_CLASSE'].'" disabled >
                                                    <input type="hidden" name="classe" value="'.$key['ID_CLASSE'].'"></td>
