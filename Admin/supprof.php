...
$id=$_SESSION['login'];
 ...
 $req4=$con->prepare("SELECT DISTINCT `Id_Prof`,`Nom_Prof`,`Prenom_Prof`, `FK_ID_CLASSE` FROM `professeur` INNER JOIN `dispenser` ON  `Id_Prof` = `FK_Id_Prof` WHERE `Id_Prof` IN (SELECT `FK_Id_Prof` FROM `dispenser` WHERE `FK_Id_Classe` is NOT NULL ) ORDER BY `Nom_Prof` ASC ");
  $req4->execute();
  $data4 = $req4->fetchAll();
              ...
              <div class="card-body">
                <div class="table-responsive">
                  <table class="table">
                    ...
                    <tbody>
                      <?php  
  foreach ($data4 as $key ) {if ($id) {
   echo' 
    <form method="POST" action="pagesuppressprof.php">

                                                <tr>
                                                <td><input type="text" name="" value="'.$key['Nom_Prof'].'" disabled >
                                                <input type="hidden" name="mat" value="'.$key['Id_Prof'].'"></td>

                                                <td><input type="text" name="" value="'.$key['Prenom_Prof'].'" disabled></td>

                                                <td><input type="text" name="" value="'.$key['FK_ID_CLASSE'].'" disabled>
                                                <input type="hidden" name="classe" value="'.$key['FK_ID_CLASSE'].'"></td>
